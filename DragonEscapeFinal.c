...
void moveDragonTowardsPlayer(int *dragonX, int *dragonY, int playerX, int playerY)
{
    ...
    {
        if (playerX > *dragonX && *dragonX < ROWS - 2)
            (*dragonX)++;
        else if (playerX < *dragonX && *dragonX > 0)
            (*dragonX)--;
    }
    else
    {
        if (playerY > *dragonY && *dragonY < COLS - 2)
            (*dragonY)++;
        else if (playerY < *dragonY && *dragonY > 0)
            (*dragonY)--;
    }
}
    ...
    while (!WindowShouldClose())
    {
        ...
        else
        {
            ...
            while (!WindowShouldClose() && !gameOver)
            {
                ...
                if (IsKeyPressed(KEY_W) && playerX > 0 && dungeon[playerX - 1][playerY] != 'O')
                    playerX--;
                else if (IsKeyPressed(KEY_S) && playerX < ROWS - 1 && dungeon[playerX + 1][playerY] != 'O')
                    playerX++;
                else if (IsKeyPressed(KEY_A) && playerY > 0 && dungeon[playerX][playerY - 1] != 'O')
                    playerY--;
                else if (IsKeyPressed(KEY_D) && playerY < COLS - 1 && dungeon[playerX][playerY + 1] != 'O')
                    playerY++;
